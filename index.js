...
app.get('/start/:zipper', (req, res) => {
    ...
    current_process = spawn('python',["continuous_construction.py", 
                            req.params.zipper, 
                            'blue', '12'] );
...
app.get('/stop', async (req, res) => {
    ...
    const { stdout, stderr } = await exec('mv ./public/images/*.jpg ' + data_dir + name, (err, stdout, stderr) => {
        if (err) {
...
io.on('connection', function (socket) {
    _socket = socket;
    _socket.emit('images', { image_top: './images/IMG_2_good.JPEG',
                            image_head: './images/IMG_6_goodhead.JPEG'});
});

app.get('/imgs', (req, res) => {
   _socket.emit('images', { image_top: req.query.path_top,
